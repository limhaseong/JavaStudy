...
1. 자바에서의 배열이란 크기와 성격이 같은 일련의 원소들이 모여
   ...
   동질적인 성격을 가진 자료들에 대해 개별적으로 자료형을 부여하지 않고
...
- 형식 및 구조
 ...
 자료형[] 변수명;               //배열 선언
 변수명 = new 자료형[요소크기]; //배열에 메모리 할당(요소크기=배열길이=배열갯수)
 변수명[인덱스]=값;             
...
//int는 int끼리의 배열, double은 double끼리의 배열이여야 됨.
//배열은 길이를 변화시킬 수 없음.
//가변배열이란? 메모리를 새로 잡아서 함.(즉 4개방이 있던 게 5개가 되는 것이 아닌 새롭게 5개 방을 할당하는 것)

public class Test079
{
	public static void main(String[] args) //String args[]이것도 가능하지만 바람직하진 않다.
	{
		// 배열 선언
		int[] arr;
		...
		// (자바는 메모리 주소값에 사용자가 직접 접근하는 것을 허용하지 않기 때문이다.)
