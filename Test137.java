...
public class Test137
{
	public static void main(String[] args)
	{
		...
		//0 -> 두 문자열이 같다(s의 값과 seoul korea의 값이 같다는 것)
		System.out.println(s.compareTo("seoul corea"));
		//8
		// c-k까지의 차이 -> defghijk -> 8
		
		//대상 문자열(s) 중
		//해당 문자열을 찾아서 원하는 형태로 수정된 문자열 반환
		s = "우리나라 대한민군 대한독립 만세";
		s = s.replaceAll("대한", "자주"); //대한이라고 써있는 걸 다 자주로 바꿔줘 //원본 데이터를 바꾸기 위해서는 s=을 넣어줘야됨
		System.out.println("처리 결과 : " + s);
		//처리 결과 : 우리나라 자주민군 자주독립 만세
		...
		//|사                 랑|
		//중간을 못파고 파낸 곳은 버린다.(가장자리 부분만 없앤다)
		//(양쪽 가장자리 공백 제거)
		...
		//23456782345678 같은 형태로 출력

		System.out.println();  //개행

		s = String.format("%d",n); //==출력이 아닌 이런 형태를 만들어라라는 것
		System.out.println(s);
		//2345678
		...
		//String strTemp = "기본,열정,배려";
		//String[] strArr = strTemp.split(",");          //split()=문자열 배열 반환하는 메소드
		//line175-176와 동일한 구문
